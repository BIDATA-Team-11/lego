 ...
 * <p>
 * The WheeledChassis provides a control system for driving a mobile robot with
 * motorized wheels. Both differential and holonomic robots can be represented
 * by the WheeledChassis class.
 * </p>
 *
 * <p>
 * <b>How to create a WheeledChassis object</b><br>
 * The constructor of the DifferentialChassis class accepts an array of Wheel
 * objects. Each of the wheel objects describes one of the motorized wheels on
 * the chassis. A Description of a wheel consists of its diameter, its position,
 * its motor and the gear train between wheel and motor. Wheel objects can be
 * created using a modeler class. A modeler for traditional wheel can be
 * obtained using the {@link #modelWheel} method. A modeler for a holonomic
 * wheel can be obtained using the {@link #modelHolonomicWheel} method.
 * </p>
 * .
 * <p>
 ...
 * </p>
 * <p>
 * Please note that a Chassis can have an unlimited number of motorized wheels
 * but a differential robot needs at least two wheels and a holonomic robot
 * needs at least three wheels.
 * </p>
 *
 * <p>
...
public class WheeledChassis implements Chassis {
  ...
  final int nWheels;
  /**
   * The program adds a dummy wheel to a differential chassis. This dummy wheel
   * makes it possible to use linear algebra on three elements (x, y, angle) for
   * the differential chassis. The y value is always zero for a differential
   * chassis.
   */
  ...
  public WheeledChassis(Wheel[] wheels, int dim) {
    ...
    if (nWheels < dim)
      throw new IllegalArgumentException(String.format("The chassis must have at least %d motorized wheels", dim));
    ...
    // create absolute versions of forward and reverse matrices (used for speed and
    // acceleration that both are always positive).
  ...
  public void setAcceleration(double linearAcceleration, double angularAcceleration) {
    if (linearAcceleration <= 0 || angularAcceleration <= 0)
  ...
  public synchronized void setVelocity(double linearSpeed, double direction, double angularSpeed) {
    if (dummyWheels == 1 && (direction % 180 != 0))
    ...
    Matrix motorAcceleration = forwardAbs
        .times(copyAbsolute(toCartesianMatrix(linearAcceleration, Math.toRadians(direction), angularAcceleration)));
    ...
    if (longestTime == 0)
      return; // Aha, no speed differences. Do nothing.
    // Devide speed difference by the longest time to get acceleration for each
    // wheel
  ...
  public void arc(double radius, double angle) {
    ...
    if (Double.isInfinite(angle)) {
      // Decrease one of both speed components so that they have the calculated ratio
      // and call travel method
      if (ratio > 1)
      ...
      Matrix displacement = toMatrix(Math.signum(angle) * 2 * Math.PI * Math.abs(radius) * Math.abs(angle) / 360, 0,
          Math.signum(radius) * angle);

      Matrix tSpeed;
      Matrix tAcceleration;
      // Decrease one of both speed and acceleration components so that they have the
      // calculated ratio
      if (ratio > 1) {
      ...
      // calculate the ratio between motor displacements when the largest displacement
      // is set to 1;
  ...
  public Move getDisplacement(Move move) {
    ...
    if (distance == 0 && rotation == 0)
      ...
      move.setValues(Move.MoveType.ARC, (float) distance, (float) rotation, isMoving());
    return move;
  }
  ...
  /**
   * The Modeler class helps to model a wheel. Wheel attributes can be modeled
   * using methods.
   * <ul>
   * <li>polarPosition() specifies the location of the wheel</li>
   * <li>gearRatio() specifies the gear ratio of the gear train between motor and
   * wheel</li>
   * <li>invert() inverts the direction of the motor. Equivalent to a negative
   * gearing</li>
   * </ul>
   * <p>
   *
  ...
  public static class HolonomicModeler implements Wheel {
     ...
     *
     * @param motor    The regulated motor that drives the wheel
     * @param diameter The diameter of the wheel (Lego wheels have the diameter
     *                 printed on the side)
     */
     ...
     *
     * @param angle  Angle between wheel axis and the robots x-axis
     * @param radius Distance between center of the wheel and center of the robot
     * @return the modeler
     */
    public HolonomicModeler polarPosition(double angle, double radius) {
      pose = new Pose((float) (radius * Math.cos(Math.toRadians(angle))),
          (float) (radius * Math.sin(Math.toRadians(angle))), (float) (angle));
      pose.rotateUpdate(90);
      return this;
    }

    /**
     * Specifies the location and orientation of the wheel using a cartesian
     * coordinates
     * <p>
     *
     * @param x     X-position of the center of the wheel in respect to the robots
     *              origin
     * @param y     Y-position of the center of the wheel in respect to the robots
     *              origin
     * @param angle Angle between the driving direction of the wheel and the robots
     *              x-axis
     * @return the modeler
     */
     ...
     *
     * @param val The ratio between wheel speed and motor speed. A ratio greater
     *            than 1 means the wheel turns faster than the motor.
     * @return the modeler
     */
    ...
    public Matrix getFactors() {
      // TODO: correct angular component when the wheel axis doesn't go through the
      // origin of the robot

      Matrix factors = new Matrix(1, 3);
      factors.set(0, 0,
          Math.cos(Math.toRadians(pose.getHeading())) * (360) / (diameter * Math.PI * gearRatio * (invert ? -1 : 1)));
      factors.set(0, 1,
          Math.sin(Math.toRadians(pose.getHeading())) * (360) / (diameter * Math.PI * gearRatio * (invert ? -1 : 1)));
   ...
   * <li>gearRatio() specifes the gear ratio of the gear train between motor and
   * wheel</li>
   * <li>invert() inverts the direction of the motor. Equivalent to a negative
   * gearing</li>
   * </ul>
  ...
  public static class Modeler implements Wheel {
     ...
     *
     * @param val The distance between the robots yPose-axis and the center of the
     *            wheel
     * @return the modeler
     */
     ...
     * Defines the gear train between motor and wheel.
     *
     * @param val The ratio between wheel speed and motor speed
     * @return the modeler
     */
  ...
  /**
   * The odometer keeps track of the robot pose based on odometry using the
   * encoders of the regulated motors of the wheels.
   *
   * @author Aswin Bouwmeester
   *
   */
...
/*
 * Some remarks regarding the design of the WheeledChassis. These originate from
 * a discussion between leJOS developers about the code of this class
 *
